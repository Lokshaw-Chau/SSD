...
class SSDUpdate(object):
    ...
    def update(self, buffer, x, y, condense_flag=True, transform=None, **kwargs):
        if not condense_flag:
            ...
            i = 0
            for data, label in zip(x, y):
                condense_indices = buffer.condense_dict[label.item()]
                buffer.buffer_img[condense_indices[i%20]].data.copy_(data)
                i += 1
            # # a dataset distill step
    ...
    def retrieve_update_model(self, x, y, buffer, transform):
        ...
        r_x, r_y = random_retrieve(buffer, self.params.batch, excl_indices=condense_indices)
